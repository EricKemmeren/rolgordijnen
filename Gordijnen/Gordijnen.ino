...
void setup() {
  ...
  Serial.println("--------- GORDIJNEN V1.0 ---------");

  // Load the saved state the table was in
  preferences.begin("gordijnen", false);

  Serial.setTimeout(500);// Set time out for

  // setup_check_window();

  Motor* gordijn = new Motor(publish_path_rolgordijnen, MOTOR_1_STEP, MOTOR_1_DIR, MOTOR_1_ENABLE, "gordijn", true);
  motors.addMotor(gordijn);
  ...
  char publish_path_rolgordijnen_on[96];
  strcpy(publish_path_rolgordijnen_on, publish_path_rolgordijnen);
  strcat(publish_path_rolgordijnen_on, "/on");
